...
void ShowInfo(char *users, int *totalPerUser, int *data, int *total, int rows, int columns);
...
int main(void){
	...
	ShowInfo(&users[0][0], totalPerUser, &data[0][0], total, USERS, COLUMNS); // Mostrar info de cada cliente
	return 0;
}
...
void ShowInfo(char *users, int *totalPerUser, int *data, int *total, int rows, int columns){
	...
	for(int i = 0; i < rows; i++){
		printf("%-15s %-10d %-10d %-10d %-10d %-10d\n",
			&users[i * MAX],                       // Apunta al nombre del usuario i
